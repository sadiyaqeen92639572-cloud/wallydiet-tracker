 ...
 * Outputs: index.html + tool-rental-db.json
 *
 * MODES:
...
function updateHtmlFile(products) {
    ...
    if (jsonIndex === -1 || jsonEndIndex === -1) {
        throw new Error('JSON markers not found in index.html');
    }
    ...
    if (rowsIndex === -1 || rowsEndIndex === -1) {
        throw new Error('Table rows markers not found in index.html');
    }
    ...
    console.log(`✅ index.html updated (${products.length} equipment)`);
}
...
async function main() {
    ...
    try {
        ...
        console.log(`🚀 Ready to deploy index.html`);
